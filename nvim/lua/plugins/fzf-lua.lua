return {
  ...
  config = function()
    ...
    require('fzf-lua').setup {
      'fzf-native',
      fzf_colors = true, -- derive fzf's colors from the editor colorscheme
      winopts = {
    ...
    --vim.keymap.set("n", "<c-F>", "<cmd>lua require('fzf-lua').git_files()<CR>", { silent = true })
    vim.keymap.set('n', '<c-g>', "<cmd>lua require('fzf-lua').live_grep()<CR>", { silent = true })
    vim.keymap.set('n', '<c-G>', "<cmd>lua require('fzf-lua').live_grep({ resume = true })<CR>", { silent = true })
